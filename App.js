      ...
      <main className="max-w-6xl mx-auto px-6">
        ...
        <section className="grid md:grid-cols-2 gap-8 items-center py-12">
          <div>
            ...
            {submitted && (
              <p className="mt-3 text-sm text-green-600">Thanks — we saved your request. We'll follow up with next steps (connect your calendar, inbox).</p>
            )}

            <div className="mt-6 text-sm text-slate-500">
              <strong>Built for:</strong> Estate agents · Letting teams · Property managers
            </div>
          </div>
...
function PricingCard({ title, price, bullets = [], highlighted = false }) {
  return (
    <div className={`${highlighted ? "border-2 border-indigo-200" : "border"} rounded-2xl p-6 bg-white`}>
...
function Testimonial({ quote, who }) {
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm">
      ...
      <p className="mt-3 text-sm text-slate-500">— {who}</p>
    </div>
  );
}
